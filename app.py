...
try:
    from ai_system.privacy import ensure_privacy_tables, safe_to_send, sanitize_text, get_user_consent, set_user_consent, export_user_data, delete_user_data, log_audit
    PRIVACY_AVAILABLE = True
except Exception as e:
    PRIVACY_AVAILABLE = False
    logger.warning(f"⚠️ No se pudo cargar ai_system.privacy: {e}")
else:
    # Si se cargó correctamente, asegurar que las tablas existan
    try:
        ensure_privacy_tables()
        logger.info("✅ Tablas de privacidad creadas/verificadas")
    except Exception as e:
        logger.warning(f"⚠️ Error creando/verificando tablas de privacidad: {e}")
...
CONFIG = {
    ...
    # Memory toggles (safe defaults: off in production)
    'MEMORY_ENABLED': os.getenv('MEMORY_ENABLED', 'false').lower() == 'true',
    'AUTO_CONTEXT_INJECTION': os.getenv('AUTO_CONTEXT_INJECTION', 'false').lower() == 'true',
...
def chat():
    ...
    try:
        ...
        # Recuperar contexto de memoria si está habilitado (solo en desarrollo/testing)
        ...
        try:
            if CONFIG.get('MEMORY_ENABLED') and MEMORY_AVAILABLE:
                ...
                ctx = get_user_memory_context(usuario, window=CONFIG.get('CONTEXT_WINDOW', 5))
                if ctx:
                    logger.info(f"🔁 Contexto recuperado para usuario {usuario}: {len(ctx)} entradas")
                    # Calcular similitud simple para decidir si inyectar
                    score = calculate_query_similarity(mensaje, ctx)
                    logger.debug(f"🔍 Similaridad mensaje-contexto: {score:.3f}")
                    if CONFIG.get('AUTO_CONTEXT_INJECTION') and score > 0.0:
                        # Concatenar contextos en forma segura antes del prompt principal
                        parts = []
                        for c in ctx:
                            parts.append(f"Usuario dijo: {c.get('pregunta')}\nRespuesta: {c.get('respuesta')}")
                        context_to_inject = "\n--- Contexto previo ---\n" + "\n\n".join(parts) + "\n--- Fin contexto ---\n"
        except Exception as e:
            logger.warning(f"⚠️ Error recuperando/inyectando memoria: {e}")

        # ✅ PROCESAR CONSULTA CON TIMEOUT ROBUSTO
        try:
            # Si tenemos contexto para inyectar, pasarlo al procesador como prefijo
            if context_to_inject:
                mensaje_procesar = context_to_inject + "\n" + mensaje
            else:
                mensaje_procesar = mensaje

            resultado = procesar_con_timeout(mensaje_procesar, timeout_segundos=REQUEST_TIMEOUT)
        ...
        # ✅ GUARDAR CONVERSACIÓN EN SQLITE SIMPLE (con checks de privacidad)
        usuario = session.get('user_id', 'anonimo') if auth_disponible else 'test_user'

        # Privacy-aware persistence: sanitizar y respetar consentimiento
        try:
            if PRIVACY_AVAILABLE:
                ok, hits = safe_to_send(mensaje)
                if not ok:
                    mensaje_safe = sanitize_text(mensaje)
                    log_audit(usuario, 'pii_redaction', details=str(hits), ip_address=client_ip)
                else:
                    mensaje_safe = mensaje

                consent = get_user_consent(usuario) if auth_disponible else False
                if consent:
                    guardar_conversacion_simple(usuario, mensaje_safe, resultado['respuesta'])
                else:
                    logger.info(f"🔒 Usuario {usuario} sin consentimiento: no se guarda conversación")
            else:
                # Si no existe el módulo privacy, comportamiento legacy
                guardar_conversacion_simple(usuario, mensaje, resultado['respuesta'])
        except Exception as e:
            logger.warning(f"⚠️ Error en checks de privacidad: {e}")
            guardar_conversacion_simple(usuario, mensaje, resultado['respuesta'])
...
def api_user_consent():
    if not auth_disponible or not is_logged_in(session):
        return jsonify({'error': 'No autorizado'}), 401
    data = request.get_json() or {}
    # Accept both 'memory_consent' and legacy 'consent' keys from the frontend
    consent = data.get('memory_consent') if 'memory_consent' in data else data.get('consent', False)
    consent = bool(consent)
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'error': 'Usuario no identificado'}), 400
    try:
        set_user_consent(user_id, consent)
        return jsonify({'status': 'ok', 'memory_consent': consent}), 200
    except Exception as e:
...
def api_user_consent_get():
    """Return the current consent status for the logged in user"""
    if not auth_disponible or not is_logged_in(session):
        return jsonify({'error': 'No autorizado'}), 401
    user_id = session.get('user_id')
    if not user_id:
    ...
    try:
        ...
        return jsonify({'status': 'ok', 'consent': bool(consent)}), 200
    except Exception as e:
        logger.error(f"❌ Error consultando consentimiento: {e}")
        return jsonify({'error': 'Error interno'}), 500
...
def api_user_data_delete():
    ...
    if not user_id:
        return jsonify({'error': 'Usuario no identificado'}), 400
    try:
        res = delete_user_data(user_id)
        return jsonify({'status': 'deleted', 'items_removed': res}), 200
    except Exception as e:
        logger.error(f"❌ Error eliminando datos: {e}")
        return jsonify({'error': 'Error interno'}), 500


@app.route('/api/user/memory-context', methods=['GET'])
def api_user_memory_context():
    if not auth_disponible or not is_logged_in(session):
        return jsonify({'error': 'No autorizado'}), 401
    ...
    user_id = session.get('user_id')
    try:
        window = int(request.args.get('window', CONFIG.get('CONTEXT_WINDOW', 5)))
        ctx = get_user_memory_context(user_id, window=window)
        return jsonify({'status': 'ok', 'context': ctx}), 200
    except Exception as e:
        logger.error(f"❌ Error obteniendo contexto de memoria: {e}")
        return jsonify({'error': 'Error interno'}), 500


@app.route('/api/user/memory-patterns', methods=['GET'])
def api_user_memory_patterns():
    if not auth_disponible or not is_logged_in(session):
        return jsonify({'error': 'No autorizado'}), 401
    if not MEMORY_AVAILABLE:
        return jsonify({'error': 'Memory module not available'}), 503
    ...
    try:
        ...
        patterns = analyze_user_patterns(user_id, top_n=top_n)
        return jsonify({'status': 'ok', 'patterns': patterns}), 200
    except Exception as e:
        logger.error(f"❌ Error analizando patrones de usuario: {e}")
        return jsonify({'error': 'Error interno'}), 500


@app.route('/api/user/data-rectify', methods=['POST'])
def api_user_data_rectify():
    """Endpoint que permite a un usuario rectificar (editar) una entrada propia en conversaciones."""
    if not auth_disponible or not is_logged_in(session):
        return jsonify({'error': 'No autorizado'}), 401
    data = request.get_json() or {}
    ...
    if not record_id or not field:
        return jsonify({'error': 'record_id y field requeridos'}), 400
    user_id = session.get('user_id')
    try:
        # Llamar a helper en privacy
        from ai_system.privacy import rectify_user_data
        res = rectify_user_data(user_id, record_id, field, new_value)
        if res.get('ok'):
            return jsonify({'status': 'ok', 'rectified': res}), 200
        else:
            return jsonify({'error': 'No autorizado o registro no encontrado'}), 403
    except Exception as e:
        logger.error(f"❌ Error rectificando datos: {e}")
        return jsonify({'error': 'Error interno'}), 500
